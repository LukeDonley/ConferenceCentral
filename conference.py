...
SPEAKER_TPL = ('Featured Speaker: %s. Sessions: %s')

# - - - Defaults and Resouce Containers - - - - - - - - - - - - - - - - -
...
# - - -  Session Objects   - - - - - - - - - - - - - - - - -
    ...
    def _createSessionObject(self, request):
        ...
        if conf.organizerUserId != user_id:
            raise endpoints.UnauthorizedException("Authorization Needed")

        
        # copy SessionForm/ProtoRPC Message into dict
        data = {field.name: getattr(request, field.name) for field in request.all_fields()}
        # initiate a task that will check if speaker should be featured
        wsckey = data['websafeConferenceKey']
        del data['sessionKey']
        ...
        taskqueue.add(params={'speaker': data['speaker'], 'wsckey': wsckey},
    ...
    @endpoints.method(SESS_GET_REQUEST, SessionForms,
            path='sessions', http_method='GET',
            name='getSessionsBySpeaker')
    def getSessionsBySpeaker(self, request):
        """Given a speaker, return all sessions given by particular speaker, across all conferences"""
        sess = []
        if getattr(request, 'speaker'):
            sess = Session.query().filter(Session.speaker == request.speaker)
        return SessionForms(items=[self._copySessionToForm(session) for session in sess])
...
# - - -  Wishlist Methods  - - - - - - - - - - - - - - - - -
    ...
    def addSessionToWishlist(self, request):
        ...
        if sk in prof.wishlistSessionKeys:
            raise ConflictException("Session already in wishlist")

        else:
            prof.wishlistSessionKeys.append(sk)
    ...
    @endpoints.method(message_types.VoidMessage, SessionForms,
            path='wishlist/sessions', http_method='GET',
            name='getSessionsInWishlist')
    def getSessionsInWishlist(self, request):
        """query for all the sessions in a conference that the user is interested in"""
        prof = self._getProfileFromUser()
        sess_keys = [ndb.Key(urlsafe=sk) for sk in prof.wishlistSessionKeys]
    ...
    def deleteSessionInWishlist(self, request):
        ...
        if not sess:
            raise endpoints.NotFoundException(
                'No wishlist entry found with key: %s' % sk)

        # check if session in wishlist
        if sk in prof.wishlistSessionKeys:
            prof.wishlistSessionKeys.remove(sk)
    ...
    @staticmethod
    def _cacheFeaturedSpeaker(featuredSpeaker, sessions):
        """Create Featured Speaker & assign to memcache"""
        feat = SPEAKER_TPL % (featuredSpeaker, ', '.join([session.name for session in sessions]))
        memcache.set(MEMCACHE_FEATURE_KEY, feat)
    ...
    def getFeaturedSpeaker(self, request):
        ...
        return StringMessage(data=memcache.get(MEMCACHE_FEATURE_KEY) or "")


# - - - Additional Queries - - - - - - - - - - - - - - - - -

    @endpoints.method(message_types.VoidMessage, ConferenceForms,
                      path='queries/getSummerConferences',
                      http_method='GET', name='getSummerConferences')
    def getSummerConferences(self, request):
        """Query for all conferences taking place between June and August"""
        q = Conference.query()
        q = q.filter(Conference.month >= 6)
        q = q.filter(Conference.month <= 8)
